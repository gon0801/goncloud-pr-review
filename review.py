...
def compose(result, manifest, *, head_sha, provider):
    ...
    parts = [MARKER, f"{SHA_PREFIX}{head_sha} -->",
             f"### Revisión automática · {provider['label']} · {head_sha[:7]}", ""]
    if warnings:
...
def cmd_publish(args):
    ...
    manifest = json.loads((work / "manifest.json").read_text())
    body = redact(compose(result, manifest, head_sha=head, provider=name),
