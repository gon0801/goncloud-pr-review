...
| B3 | Candado determinista contra falsos "resuelto": el "resuelto" del modelo se acepta cuando cita el cambio concreto que elimina la causa, aunque viva en otro archivo (p. ej. el arreglo está en el punto de llamada, no donde se reportó). El candado de "su archivo cambió" solo aplica cuando ningún cambio del push explica el resuelto: en ese caso sigue abierto diga lo que diga el modelo. Si el cambio que lo causó ya no está en el PR (revert), se resuelve solo. | cc:TODO |
...
| B6 | Si el modelo no entrega el bloque de hallazgos bien formado, se cae a la revisión completa actual. Nunca se pierde una revisión por esto. Prueba unitaria: bloque bien formado sobrevive al parseo de cobertura y al recorte; bloque ausente o roto cae a revisión completa. | cc:TODO |
